  ...
  Object.entries(args).forEach(([arg, val]) => {
    ...
    // e.g., where: [{ path: "age", op: ">=", val: "$fields{minAge}" }]
      ...
      if (Array.isArray(val)) {
        val.forEach((elem) => preprocessExports(elem, fields, ref));
      } else {
        // technically this case never happens, but it could be useful for
        // future directives
        preprocessExports(val, fields, ref);
      }
    }
  });
...
export function createQuery(
  ...
  return applyOperations(
    ...
    // limit
    operation(args.limit, (query, limit) => query.limit(Number(limit))),
  );
}
